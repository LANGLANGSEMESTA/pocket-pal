...
import { ArrowLeft, LogOut, Copy as CopyIcon, Sparkles } from "lucide-react";
...
const Settings = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: prof }, { data: bud }] = await Promise.all([
        supabase.from("profiles").select("username,home_currency,preferensi_bahasa").eq("id", user.id).maybeSingle(),
        supabase.from("budgets").select("total_limit,alert_threshold").eq("user_id", user.id).eq("year", now.getFullYear()).eq("month", now.getMonth() + 1).maybeSingle(),
      ]);
      if (prof) {
  ...
  const saveProfile = async (patch: Partial<{ username: string; home_currency: string; preferensi_bahasa: string }>) => {
    if (!user) return;
  ...
  const resetBudget = async () => {
    if (!user) return;
    const now = new Date();
    await supabase.from("budgets").delete().eq("user_id", user.id).eq("year", now.getFullYear()).eq("month", now.getMonth() + 1);
    setBudget(0);
    setBudgetInput("");
    setResetOpen(false);
  ...
  const savePayment = () => {
    if (!pay.bank || !pay.accountNumber || !pay.accountName) return toast.error("Lengkapi semua field");
    setPaymentInfo(pay);
    ...
    <div className="app-shell pb-28">
      ...
      <main className="px-4 py-4 space-y-5">
        ...
        <Section title="Profil">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              <Input value={username} onChange={(e) => setUsername(e.target.value)} onBlur={() => saveProfile({ username })} />
            </div>
          </div>
          <div className="space-y-2">
            <div>
              <Label className="text-xs">Bahasa</Label>
              <Select value={lang} onValueChange={(v) => { setLang(v); saveProfile({ preferensi_bahasa: v }); }}>
            ...
            <div>
              ...
              <Select value={currency} onValueChange={(v) => { setCurrency(v); saveProfile({ home_currency: v }); }}>
                <SelectTrigger><SelectValue /></SelectTrigger>
        ...
        <Section title="Keuangan">
          <Card>
            <CardContent className="p-3 flex justify-between items-center">
              ...
              <Button size="sm" variant="outline" onClick={() => setBudgetEdit(true)}>Edit</Button>
            </CardContent>
          </Card>
          <Button variant="ghost" className="w-full text-danger mt-2" onClick={() => setResetOpen(true)}>
        ...
        <Section title="Informasi Pembayaran">
          <p className="text-xs text-muted-foreground mb-2">🔒 Disimpan di device kamu saja, tidak ke server.</p>
          <div className="space-y-2">
            <Input placeholder="Bank / E-wallet" value={pay.bank} onChange={(e) => setPay({ ...pay, bank: e.target.value })} />
            <Input placeholder="Nomor rekening / HP" value={pay.accountNumber} onChange={(e) => setPay({ ...pay, accountNumber: e.target.value })} />
            <Input placeholder="Nama pemilik" value={pay.accountName} onChange={(e) => setPay({ ...pay, accountName: e.target.value })} />
            <Button size="sm" onClick={savePayment} className="w-full">Simpan</Button>
          </div>
        </Section>
          ...
          {code ? (
            <Card>
              <CardContent className="p-4 text-center space-y-2">
                ...
                {codeExp && <div className="text-xs text-muted-foreground">Berlaku sampai {codeExp.toLocaleString("id-ID")}</div>}
                <Button size="sm" variant="outline" onClick={copyCode} className="w-full"><CopyIcon className="h-3.5 w-3.5 mr-1" /> Salin Kode</Button>
              </CardContent>
            </Card>
          ) : (
            <Button onClick={generateInviteCode} className="w-full" variant="outline">Generate Kode (24 jam)</Button>
          )}
        </Section>
        ...
        <Section title="Berlangganan">
          <Card className="border-primary/30">
            <CardContent className="p-4 space-y-2">
              ...
              <Button onClick={() => setProOpen(true)} className="w-full">Upgrade ke Pro — Rp 15.000/bulan</Button>
            </CardContent>
          </Card>
        </Section>

        {/* KELUAR */}
        <Button variant="outline" className="w-full text-danger border-danger/30 hover:bg-danger-soft" onClick={() => setLogoutOpen(true)}>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input inputMode="numeric" value={budgetInput} onChange={(e) => setBudgetInput(e.target.value.replace(/\D/g, ""))} />
            </div>
            <div>
              <Label>Ingatkan saat {thresholdInput}% terpakai</Label>
              <Slider min={50} max={90} step={5} value={[thresholdInput]} onValueChange={(v) => setThresholdInput(v[0])} />
            </div>
          </div>
          <DialogFooter>
      ...
      <AlertDialog open={resetOpen} onOpenChange={setResetOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Budget bulan ini akan dihapus. Transaksi tidak terpengaruh.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Batal</AlertDialogCancel>
            <AlertDialogAction onClick={resetBudget} className="bg-danger text-danger-foreground hover:bg-danger/90">Reset</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={logoutOpen} onOpenChange={setLogoutOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Batal</AlertDialogCancel>
            <AlertDialogAction onClick={doLogout} className="bg-danger text-danger-foreground hover:bg-danger/90">Keluar</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Dialog open={proOpen} onOpenChange={setProOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Fitur Pro segera hadir! Kamu akan dapat: scan struk unlimited, split per item, prediksi stok pintar, export laporan, dan banyak lagi.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
